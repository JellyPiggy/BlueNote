...
| `bluenote_rank` | `bluenote-rank` | 榜单定义、笔记轻量索引、分数、贡献、快照、消费幂等 |
...
Rank 第一阶段物理部署在 `bluenote-social-app`，但使用独立 schema 和包结构。

### 7.1 rank_definition

榜单定义表，第一阶段初始化固定两条：`WEEKLY_HOT_NOTE`、`YEARLY_CREATOR_GROWTH`。

字段：`rank_code`、`rank_name`、`member_type`、`period_type`、`score_rule_json`、`top_limit`、`redis_limit`、`estimate_enabled`、`public_visible`、`definition_status`、`created_at`、`updated_at`。

索引：`PRIMARY KEY(rank_code)`。

### 7.2 rank_period

榜单周期表。

字段：`id`、`rank_code`、`period_id`、`period_type`、`period_status`、`start_at`、`end_at`、`frozen_at`、`created_at`、`updated_at`。

索引：`uk_rank_period(rank_code, period_id)`、`idx_rank_period_status(rank_code, period_status, start_at)`。

### 7.3 rank_note_index

排行榜笔记轻量索引。

字段：`note_id`、`author_id`、`title_snapshot`、`cover_url_snapshot`、`visibility`、`note_status`、`eligible_status`、`published_at`、`last_event_id`、`created_at`、`updated_at`。

索引：`PRIMARY KEY(note_id)`、`idx_rank_note_author(author_id, published_at)`、`idx_rank_note_eligible(eligible_status, updated_at)`。

### 7.4 rank_member_score

成员周期分数表，是 Redis 榜单的恢复依据。

字段：`id`、`rank_code`、`period_id`、`member_type`、`member_id`、`score`、`rank_score`、`score_version`、`member_status`、`last_score_at`、`created_at`、`updated_at`。

索引：`uk_rank_member_score(rank_code, period_id, member_type, member_id)`、`idx_rank_score_top(rank_code, period_id, score, member_id)`。

### 7.5 rank_score_contribution

来源贡献表，用于笔记不可见时扣回或恢复贡献。

字段：`id`、`rank_code`、`period_id`、`member_type`、`member_id`、`source_type`、`source_id`、`score`、`contribution_status`、`last_event_id`、`created_at`、`updated_at`。

索引：`uk_rank_contribution(rank_code, period_id, member_type, member_id, source_type, source_id)`、`idx_rank_contribution_source(source_type, source_id, period_id)`。

### 7.6 rank_score_change_log

分数变化流水和幂等表。

字段：`change_id`、`source_event_id`、`source_event_type`、`rank_code`、`period_id`、`member_type`、`member_id`、`source_type`、`source_id`、`field_name`、`delta_value`、`score_delta`、`score_after`、`occurred_at`、`created_at`。

索引：`PRIMARY KEY(change_id)`、`uk_rank_score_change(source_event_id, rank_code, member_type, member_id, field_name)`、`idx_rank_score_log_period(rank_code, period_id, occurred_at)`。

### 7.7 rank_snapshot / rank_snapshot_item

榜单快照表和快照明细表。

`rank_snapshot` 字段：`snapshot_id`、`rank_code`、`period_id`、`snapshot_type`、`item_count`、`source_version`、`created_at`。

`rank_snapshot_item` 字段：`id`、`snapshot_id`、`rank_no`、`member_type`、`member_id`、`score`、`rank_score`、`created_at`。

索引：`idx_rank_snapshot_latest(rank_code, period_id, snapshot_type, created_at)`、`idx_rank_snapshot_item(snapshot_id, rank_no)`。

### 7.8 rank_event_consume_log / rank_outbox_event / rank_rebuild_task

`rank_event_consume_log` 用于 `note-event`、`counter-event` 消费幂等，唯一约束 `uk_rank_consume(consumer_group, event_id)`。

`rank_outbox_event` 支持 `RankChanged`、`RankFrozen`，索引同通用 outbox 要求。

`rank_rebuild_task` 字段：`task_id`、`task_type`、`rank_code`、`period_id`、`task_status`、`progress_json`、`reason`、`error_message`、`created_at`、`updated_at`。

索引：`idx_rank_task_status(task_status, created_at)`。

## 8. bluenote_notification

### 8.1 notification_record
...
### 8.2 notification_aggregate_actor
...
### 8.3 notification_unread_counter
...
### 8.4 notification_outbox_event / notification_consume_record
...
### 9.6 push_consume_record / push_outbox_event
...
### 10.3 im_message / im_conversation_message
...
### 10.4 im_user_sequence / im_user_message
...
### 10.5 im_outbox_event / im_consume_record
