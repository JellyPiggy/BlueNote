...
版本：v0.15
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/notification/push 完成 foundation，第三条实时链路 IM 单聊最小纵切面已接入，第四条订单链路 foundation 已接入
更新时间：2026-06-12
当前分支：`codex/order-foundation`
当前基线提交：以本分支最新提交为准
...
| order-app | 已新增神券订单 foundation | 活动配置/预热、秒杀 token、Redis Lua 预扣、异步下单、免费券发券、MOCK 支付、取消待支付订单、超时关单兜底、订单 outbox 和 MQ listener 已接入 |
...
限制：订单 foundation 目前只做神券秒杀最小闭环；真实支付渠道、退款/撤销发券、运营后台、库存对账报表、Redis 重建运维接口和高并发压测仍待增强。通用 outbox 还没有 RocketMQ 延时级别，超时关单依赖即时检查加定时扫表兜底。
...
已接入订单契约、DDL、后端 app、网关路由、MQ/outbox 和移动端神券页。后续应优先补真实支付渠道、运营后台、库存对账/Redis 重建运维接口、订单通知展示和并发压测。
...
8. order-app 当前已完成神券秒杀 foundation，但未做真实支付、退款、运营后台和高并发压测。
