...
版本：v0.11
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/notification 持续落地，移动端通知中心、我的页互动列表和 content 笔记 counter 读计数已接入
更新时间：2026-06-12
当前分支：`codex/note-counter-reads`
当前基线提交：以本分支最新提交为准
...
限制：notification 已完成站内通知最小闭环，移动端通知页和消息入口未读角标已接入；还没有用户通知偏好、通知生命周期归档任务和 Push 服务实际投递。
