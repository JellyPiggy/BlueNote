...
版本：v0.7
状态：第一条主链路完成，第二条社交链路 relation/counter/feed 持续落地，feed 写模型和 MQ 消费已接入
更新时间：2026-06-12
当前分支：`codex/feed-foundation`
当前基线提交：以本分支最新提交为准
...
| social-app | 已新增 relation/counter/feed 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/自动 MQ 消费、feed 收件箱/fanout/重建/自动 MQ 消费已落地 |
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter/feed 自动 listener 已落地 |
...
3. 通用 outbox dispatcher 扫描各应用声明的 outbox 表：
   ...
   - social：`relation_outbox_event`、`counter_outbox_event`、`feed_outbox_event`
...
2. 当前自动 listener 已接 counter/feed；notification 的 MQ 消费仍待通知读模型落地时接入。
...
3. 笔记发布、关注事件已可驱动 feed 写模型；点赞、收藏、评论当前只驱动 counter，notification 仍未消费这些事件。
4. 自动化测试基本还没建立，后续改动风险会越来越高。
5. social-app 当前已完成 relation、counter 和 feed 最小纵切面，notification 仍未接入。
