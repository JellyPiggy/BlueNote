...
版本：v0.6
状态：第一条主链路完成，第二条社交链路 relation/counter/feed 持续落地，MQ/outbox 基座已接入
更新时间：2026-06-12
当前分支：`codex/mq-outbox-foundation`
当前基线提交：以本分支最新提交为准
...
| social-app | 已新增 relation/counter/feed 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/事件入口/自动 MQ 消费、关注页 Feed 读路径已落地 |
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter 自动 listener 已落地 |
...
限制：笔记详情页点赞数、收藏数当前仍直接从互动明细聚合，详情页评论数还没有统一切到 counter；feed、notification 链路还没有完整事件消费和读模型闭环。

### 3.7 Social App

`bluenote-social-app` 当前包含 relation/counter/feed 第二条主链路起步能力。
...
2. 当前读路径基于 relation 当前关注列表实时拉取作者近期公开笔记。
3. Feed 卡片聚合 member 用户摘要、note 摘要、counter 计数。
4. 按契约返回 `publishedAtMillis_noteId` 游标、`PULL` 来源、`viewerAction=null` 和降级标记。
5. gateway 已增加 `/api/feed/**` 路由，并保持鉴权访问。

限制：counter 已接入真实 RocketMQ listener 和通用 outbox dispatcher；feed 当前仍是基于关注作者近期公开笔记的实时拉取窗口，还没有落收件箱快照、发布事件 fanout、Redis 读模型、重建任务和 feed outbox。
...
3. 通用 outbox dispatcher 扫描各应用声明的 outbox 表：
   ...
   - social：`relation_outbox_event`、`counter_outbox_event`
...
2. 当前自动 listener 先接 counter；feed/notification 的 MQ 消费仍待各自业务读模型落地时接入。
...
2. 我的收藏列表和赞过列表。
3. feed 服务：发布事件投递、收件箱快照、Redis 读模型和重建任务。
4. notification 服务：互动通知和未读数。
5. content 笔记详情、列表等读接口逐步切换到 counter 计数。
6. Feed key 的真实读写和重建策略。
...
3. 笔记点赞、收藏已落库并写 outbox，counter 可通过 RocketMQ 自动消费；feed、notification 仍未消费这些事件。
4. 自动化测试基本还没建立，后续改动风险会越来越高。
5. social-app 当前已完成 relation、counter 和 feed 读路径纵切面，feed 写模型/notification 仍未接入。
