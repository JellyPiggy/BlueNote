...
状态：第一条主链路阶段交接稿  
更新时间：2026-06-11  
当前分支：`codex/bootstrap-main-chain`  
当前基线提交：`2774283 Refine mobile profile header layout`
...
BlueNote 已经从纯方案阶段推进到“第一条主链路可联调基线”阶段。
...
| 范围 | 状态 | 说明 |
|---|---|---|
| 契约 | 已建立第一条主链路基线 | API、错误码、DDL、Redis、MQ、安全权限文档已进入 `docs/contracts/` |
| 本地依赖 | 已建立 local Compose | MySQL、Redis、Nacos、RocketMQ、MinIO 使用常用本地端口 |
| 后端基础 | 已搭建 Maven 多模块 | common、gateway、member、content 已有可执行结构 |
| member-app | 已从内存占位改为 MySQL | auth/user 注册、登录、Token、用户资料等主链路接口已落库 |
| gateway-app | 已补 JWT 校验 | 网关校验 Access Token 并注入用户上下文 Header |
| content-app | 已从占位接口接入 MySQL/MinIO | file/note 可申请上传凭证、确认文件、发布笔记、查询详情 |
...
限制：点赞、收藏、评论的写接口和完整计数链路还没有落地；当前详情页可以展示计数字段和 viewerAction 结构，但互动行为本身仍是下一阶段任务。
...
本文创建时没有重新执行后端完整编译和端到端接口联调。后续继续后端或联调任务前，建议先补一次 backend compile、服务启动和主链路接口验证。
...
1. relation 服务或 social-app 基础工程。
2. counter 服务：关注数、粉丝数、获赞数、点赞数、收藏数、评论数。
3. note 点赞/取消点赞、收藏/取消收藏接口。
4. comment 服务：一级评论、回复、删除、评论列表。
5. feed 服务：关注页 Feed 拉取和发布事件投递。
6. notification 服务：互动通知和未读数。
7. RocketMQ outbox dispatcher 和消费幂等记录。
8. Redis 计数、Feed key 的真实读写和重建策略。
...
3. 点赞、收藏、评论的页面展示已经有一部分 UI，但后端写接口和真实行为链路未完成。
4. 自动化测试基本还没建立，后续改动风险会越来越高。
5. `backend/README.md` 已落后于当前实现，需要尽快更新。
