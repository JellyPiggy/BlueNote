...
版本：v0.5
状态：第一条主链路完成，第二条社交链路 relation/counter/feed 持续落地
更新时间：2026-06-12
...
| social-app | 已新增 relation/counter/feed 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/事件入口、关注页 Feed 读路径已落地 |
...
限制：counter 的消费核心已落地，但项目还没有统一 RocketMQ listener 和 outbox dispatcher，因此现在通过内部事件消费入口承接后续 MQ listener、补偿脚本或本地联调。feed 当前是基于关注作者近期公开笔记的实时拉取窗口，还没有落收件箱快照、发布事件 fanout、Redis 读模型、重建任务和 feed outbox。
...
1. RocketMQ outbox dispatcher、自动 listener、重试和死信闭环。
2. 我的收藏列表和赞过列表。
...
1. Outbox 当前主要是写库记录，还没有完整 MQ dispatcher、重试和死信闭环。
2. counter 消费核心、快照、Redis 和 outbox 已落地，但还没有接入真实 RocketMQ listener 自动消费。
3. 笔记点赞、收藏已落库并写 outbox，counter 可通过内部事件入口消费；feed、notification 仍未消费这些事件。
