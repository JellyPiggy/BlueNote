...
版本：v0.15
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/notification/push 完成 foundation，第三条实时链路 IM 单聊最小纵切面已接入，第四条订单链路 foundation 和库存可靠性最小闭环已接入
更新时间：2026-06-12
当前分支：`codex/order-reliability-foundation`
当前基线提交：以本分支最新提交为准
...
BlueNote 已经从纯方案阶段推进到多条主链路可联调基线阶段，并开始落地第四条神券订单链路。
...
| 范围 | 状态 | 说明 |
|---|---|---|
| 契约 | 已建立第一、第二、第三条主链路基线，并新增第四条订单链路 foundation 契约 | API、错误码、DDL、Redis、MQ、安全权限文档已进入 `docs/contracts/` |
...
| order-app | 已新增神券订单 foundation 和库存可靠性最小闭环 | 活动配置/预热、秒杀 token、Redis Lua 预扣、异步下单、免费券发券、MOCK 支付、取消待支付订单、超时关单兜底、订单 outbox、MQ listener、活动运维摘要、Redis 库存重建、库存对账修复和卡住请求收敛已接入 |
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter/feed/notification/push/order 自动 listener 已落地，IM 和订单 outbox 已接入通用 dispatcher |
| mobile | 已接入 H5 主链路、通知中心、realtime、私信和神券最小页 | 登录、首页、发布、详情、我的页、消息页已接真实 API，登录后自动注册 push 设备并连接 WebSocket；私信列表/聊天页和神券页已接真实 API |
...
已完成第一、第二、第三条主链路契约基线，并新增第四条订单链路 foundation 契约：
...
限制：订单 foundation 目前只做神券秒杀最小闭环；真实支付渠道、退款/撤销发券、运营后台页面、库存对账报表和高并发压测仍待增强。通用 outbox 还没有 RocketMQ 延时级别，超时关单依赖即时检查加定时扫表兜底。
...
9. order-app 新增 order MQ listener，自动消费 `order-seckill-task-event` 和 `order-timeout-event`。
10. 新增内部运维接口：
...
2. 当前自动 listener 已接 counter/feed/notification/push/order；IM 目前是业务事件和 Push 请求生产方，后续再消费 push 回执。
...
已接入订单契约、DDL、后端 app、网关路由、MQ/outbox、移动端神券页、库存对账/Redis 重建运维接口和卡住请求收敛。后续应优先补真实支付渠道、运营后台页面、订单通知展示和并发压测。
...
4. 订单 `PushSendRequested` 已写出 outbox，push 服务可继续按现有 `push-request-event` 消费；订单通知读模型后续可再消费 `order-event` 增强站内通知。
5. 订单超时关单当前依赖即时 `OrderTimeoutCheck` 加扫表兜底；通用 outbox 尚未支持 RocketMQ 延时级别。
6. 自动化测试基本还没建立，后续改动风险会越来越高。
7. social-app 当前已完成 relation、counter、feed、notification、push 和 im 最小纵切面。
8. order-app 当前已完成神券秒杀 foundation 和库存可靠性最小闭环，但未做真实支付、退款、运营后台页面和高并发压测。
