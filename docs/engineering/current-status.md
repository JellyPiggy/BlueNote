...
版本：v0.12
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/notification/push 持续落地，移动端通知中心、我的页互动列表和 content 笔记 counter 读计数已接入
更新时间：2026-06-12
当前分支：`codex/push-realtime-foundation`
当前基线提交：以本分支最新提交为准
...
| social-app | 已新增 relation/counter/feed/notification/push 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/自动 MQ 消费、feed 收件箱/fanout/重建/自动 MQ 消费、通知读模型/未读数/自动 MQ 消费、push 设备/偏好/投递请求/投递日志已落地 |
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter/feed/notification/push 自动 listener 已落地 |
| mobile | 已接入 H5 主链路和通知中心 | 登录、首页、发布、详情、我的页、消息页已接真实 API，首页/我的页有未读角标 |
...
`bluenote-social-app` 当前包含 relation/counter/feed/notification/push 第二条主链路起步能力。
...
3. 已落 `bluenote_push.push_device`、`push_preference`、`push_delivery_request`、`push_delivery_attempt`、`push_click_log`、`push_consume_record`、`push_outbox_event`。
...
6. 当前投递通道为 `NOOP` 基座：可生成请求、尝试、过滤和结果 outbox，尚未接真实 WebSocket / uni-push / 厂商 Push。
...
3. 通用 outbox dispatcher 扫描各应用声明的 outbox 表：
   ...
   - social：`relation_outbox_event`、`counter_outbox_event`、`feed_outbox_event`、`notification_outbox_event`、`push_outbox_event`
...
2. 当前自动 listener 已接 counter/feed/notification/push；后续还缺死信告警、人工重放审计和真实 WebSocket / uni-push 投递通道。
...
3. push 真实 WebSocket / uni-push 投递通道、ACK 和移动端设备注册联调。

### 6.4 后续链路

第三条实时链路待完成，当前只完成 push 设备、偏好和投递请求基座，真实 WebSocket / uni-push 和 IM 仍未落地：
...
3. 笔记发布、关注事件已可驱动 feed 写模型；点赞、收藏、评论、关注和笔记状态事件已可驱动 notification 读模型；`PushSendRequested` 已可驱动 push 投递请求和 NOOP 投递日志。
4. 自动化测试基本还没建立，后续改动风险会越来越高。
5. social-app 当前已完成 relation、counter、feed、notification 和 push 最小纵切面。
