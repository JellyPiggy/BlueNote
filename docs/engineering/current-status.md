...
版本：v0.17
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/rank/notification/push 完成 foundation，第三条实时链路 IM 单聊最小纵切面已接入，第四条订单链路 foundation、库存可靠性和活动运营最小闭环已接入
更新时间：2026-06-12
当前分支：`codex/rank-foundation`
当前基线提交：以本分支最新提交为准
...
BlueNote 已经从纯方案阶段推进到多条主链路可联调基线阶段，并开始落地排行榜 foundation。
...
| social-app | 已新增 relation/counter/feed/rank/notification/push/im 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/自动 MQ 消费、feed 收件箱/fanout/重建/自动 MQ 消费、rank 周热笔记/年度创作者成长榜、通知读模型/未读数/自动 MQ 消费、push 设备/偏好/WebSocket 实时投递/投递日志、IM 单聊文字消息/未读/已读/送达/outbox 已落地 |
...
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter/feed/rank/notification/push/order 自动 listener 已落地，IM、rank 和订单 outbox 已接入通用 dispatcher |
...
限制：rank 已完成 foundation 最小闭环；后续还需要移动端榜单页、定时快照/冻结任务、历史榜单查询、运营后台入口、榜单校准报表和更完整的排行策略调参。
...
8. social-app 新增 notification MQ listener，自动消费 interaction、comment、relation 和 note 状态事件。
...
2. 当前自动 listener 已接 counter/feed/rank/notification/push/order；IM 目前是业务事件和 Push 请求生产方，后续再消费 push 回执。
...
3. rank 定时快照/冻结任务、历史榜单查询、榜单策略调参、移动端榜单页和运营后台入口。
...
已接入订单契约、DDL、后端 app、网关路由、MQ/outbox、移动端神券页、库存对账/Redis 重建运维接口、卡住请求收敛、活动列表/详情、上线前预检查、运营库存调整和状态约束。后续应优先补真实支付渠道、运营后台页面、订单通知展示和并发压测。
...
已接入 rank 契约、DDL、Redis key、MQ 事件、权限矩阵、后端模块、网关路由、MQ listener、`RankChanged` outbox、快照和重建。后续应优先补移动端榜单页、定时任务、历史榜单和运营后台。
