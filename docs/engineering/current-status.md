...
版本：v0.8
状态：第一条主链路完成，第二条社交链路 relation/counter/feed/notification 持续落地，通知读模型和 MQ 消费已接入
更新时间：2026-06-12
当前分支：`codex/notification-foundation`
当前基线提交：以本分支最新提交为准
...
| social-app | 已新增 relation/counter/feed/notification 纵切面 | 关注、取关、关注/粉丝列表、关注状态、counter 快照/Redis/重建/自动 MQ 消费、feed 收件箱/fanout/重建/自动 MQ 消费、通知读模型/未读数/自动 MQ 消费已落地 |
| MQ/outbox | 已接入基础闭环 | 通用 outbox dispatcher、RocketMQ producer/consumer 容器、counter/feed/notification 自动 listener 已落地 |
...
限制：笔记详情页点赞数、收藏数当前仍直接从互动明细聚合，详情页评论数还没有统一切到 counter；notification 已有最小可用通知读模型，但移动端通知页尚未接入。

### 3.7 Social App

`bluenote-social-app` 当前包含 relation/counter/feed/notification 第二条主链路起步能力。
...
8. 通知创建后可靠写出 `PushSendRequested` outbox，推送服务尚未落地时只保证 outbox 发送。
...
限制：notification 已完成站内通知最小闭环，但还没有移动端通知页、用户通知偏好、通知生命周期归档任务和 Push 服务实际投递。
...
3. 通用 outbox dispatcher 扫描各应用声明的 outbox 表：
   ...
   - social：`relation_outbox_event`、`counter_outbox_event`、`feed_outbox_event`、`notification_outbox_event`
...
2. 当前自动 listener 已接 counter/feed/notification；后续还缺死信告警、人工重放审计和 Push 服务实际消费。
...
2. 我的收藏列表和赞过列表。
3. 移动端通知页和消息入口未读角标。
4. content 笔记详情、列表等读接口逐步切换到 counter 计数。
5. feed 大 V 推拉结合策略、清理任务后台化、失败任务批量补偿和完整运维审计。
...
3. 笔记发布、关注事件已可驱动 feed 写模型；点赞、收藏、评论、关注和笔记状态事件已可驱动 notification 读模型。
4. 自动化测试基本还没建立，后续改动风险会越来越高。
5. social-app 当前已完成 relation、counter、feed 和 notification 最小纵切面。
