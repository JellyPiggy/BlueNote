...
版本：v0.4
状态：第一条主链路完成，第二条社交链路 relation/counter/feed 读路径起步
更新时间：2026-06-12
...
| social-app | 已新增 relation/counter/feed 最小纵切面 | 关注、取关、关注/粉丝列表、关注状态、计数聚合、关注页 Feed 读路径已落地 |
...
限制：counter 还没有消费 MQ 事件，也没有落 `counter_snapshot`、Redis 在线计数、重建任务和 `CounterChanged` outbox。feed 当前是基于关注作者近期公开笔记的实时拉取窗口，还没有落收件箱快照、发布事件 fanout、Redis 读模型、重建任务和 feed outbox。
...
9. 本轮新增 feed 读路径后，`cd backend && mvn -q -DskipTests compile` 通过。
