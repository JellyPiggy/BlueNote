...
BlueNote 是一个面向全栈/后端学习的内容社区系统。它不是一个只停留在 Controller CRUD 的练手仓库，而是按真实互联网业务的方式拆分了用户、内容、评论、关注、Feed、计数、通知、推送、IM、订单和排行榜等模块，并把 API、数据库、Redis、MQ 和权限边界写成了可核对的契约。

项目当前定位是：**个人可运行、契约优先、模块完整、适合学习后端相关知识**。

如果你想找一个可以按模块学习、可以搞清楚架构取舍的项目，BlueNote 的重点价值在这里：

1. 它**覆盖了常见社区产品后端的主要业务链路**。
...
当前项目更适合学习和展示后端设计，不适合直接作为生产系统上线。真实生产还需要补齐监控告警、真实支付、真实离线 Push、推荐搜索、审核后台和部署安全等能力。

## 项目截图

下面的图片来自本地 uni-app H5 页面。

个人信息页：

![BlueNote 个人信息页面](docs/assets/showcase/hero.png)

首页：

![BlueNote 首页](docs/assets/showcase/home.png)

 评论/点赞/收藏：

![BlueNote 评论页面](docs/assets/showcase/comment.png)
...
![BlueNote 排行榜页面](docs/assets/showcase/rank.png)
